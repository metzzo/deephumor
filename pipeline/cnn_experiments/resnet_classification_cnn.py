...
class ResNetClassificationCNNModel(BaseCNNModel):
    class Network(nn.Module):
        def __init__(self):
            super(ResNetClassificationCNNModel.Network, self).__init__()
    ...
    def get_network_class(self):
        return ResNetClassificationCNNModel.Network
    ...
    def train_evaluations(self):
        return super(ResNetClassificationCNNModel, self).train_evaluations + [AccuracyEvaluation]

    @property
    def validation_evaluations(self):
        return super(ResNetClassificationCNNModel, self).validation_evaluations + [AccuracyEvaluation] + [partial(DetailedEvaluation, model=self)]

    def get_human_readable_class(self, cl, is_predicted):
        if is_predicted:
            return Classes[cl]
        else:
            return ObjectDataset.Classes[cl].ljust(20)
