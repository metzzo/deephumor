...
def preprocess_image(path):
    ...
    top = y
    left = x
    right = original_img_width - (x + w)
    bottom = original_img_width - (y + h)
    ...
    original_img_height, original_img_width = blurred.shape
    ...
    out[mask == 255] = blurred[mask == 255]  # copy images over where mask is 255
    ...
    x, y, w, h = cv2.boundingRect(max_contour)
    punchline = blurred[y + h:y + h + (original_img_height - (y + h)), 0:original_img_width]
