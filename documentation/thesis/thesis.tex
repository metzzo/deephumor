...
A perceptron is a binary classifier and uses a linear function to perform the classification. In its base form it can distinguish between two states. The learning process starts with a random decision boundary, which is iteratively adapted such that the number of misclassifications is minimized. Due to the linear decision boundary the perceptron is not able to learn many classes of problems. For example a simple XOR operation can not be learned by a perceptron. This motivated the use of multi layer perceptrons, since they are in tehory able to approximate every function.

Historically Perceptrons are the basis for feed forward neural networks. Feedforward neural networks can be considered as multiple layers of perceptrons connected: Multi layer perceptron.
...
h(x) = max(0, x)
\end{equation}
\item SoftMax: Does not take a single input, but multiple inputs. Can be used to transform any inputs into a probability distribution.
\begin{equation}
