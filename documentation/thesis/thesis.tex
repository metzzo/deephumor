...
A confusion matrix of each classifier in the first stage reveals whether this approach could be effective. If all classifiers have a reasonable accuracy the second decision stage has a chance of predicting the correct class. Otherwise the second decision would not be able to beat the average baseline.
...
In general the conclusion is that no model really learns to generalize the humor of
Gary Larson. There is no significant improvement compared to the baselines available.
Further research into this direction could reveal significant insights into the human humor, as current methods are not capable to achieve significant results.
...
cartoon is very subjective and likely depends on many factors. For example: Current mood, whether the person has already seen the cartoon, time of day and also the exact reason for performing the annotation. It could be that these factors outweigh the true humor, which makes it impossible to reproduce the original ratings. To answer these questions an additional experiment could be performed: After some time after the initial ratings have been performed, the same person rates the cartoons again. If the ratings are not even reproducible by the same annotator, then it seems to be impossible for a deep learning based model.
