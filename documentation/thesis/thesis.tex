...
This chapter aims to evaluate the performance of the implemented architectures. I will outline the strengths and weaknesses of each implemented architecture using confusion matrices, qualitative analysis and metrics (mean absolute error and accuracy). I will also perform a performance evaluation.
...
The result of the object detection approach are incomparable to the other results, as it was not implemented such that it predicted any funniness class. Due to inconsistent detections of objects there was no virtually no possibility that this approach could achieve significant results.
...
Against the expectation that the convolutional autoencoder would improve the results due to a significantly reduced input size, it did not improve the results compared to previous attempts.

Comparing the reconstructed cartoon with the original cartoon reveals that the autoencoder looses important semantic information which could be crucial for understanding humor. Importantly, the facial expressions are lost, as well as many other high frequency details. For example the native American lying on the floor is not recognizable anymore in figure \ref{fig:autoencoderresults}. 
