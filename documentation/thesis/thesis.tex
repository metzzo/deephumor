...
All results listed are from the validation set, since no result has significant improvements compared to the baseline which would justify retraining the models on the test set.
If a trained model does not generalize enough for the validation set, then is very unlikely the trained model will generalize for the test set.

\section{Architectures}
The following architectures will be evaluated.
...
\item Most Frequent Funniness
\item Average Funniness
\item Random Funniness

\end{itemize}

As expected, for the mean absolute error metric the average performed best, while for accuracy the most frequent class has the best score.

\subsection{Simple CNN}
The simple CNN architecture overfits very quickly. The performance approximates the baseline in the classification setting, but does not even reach the baseline in the regression setting.

\subsection{Transfer Learning of Pretrained ResNet18}
When applying transfer learning of a previously pretrained ResNet18 model overfitting is not as big of a problem, but instead it is underfitting. This is one of the few instances where the model performs better than the baseline in the classification setting. But the improvement is very slim and not significant.

\subsection{Object Detection}

The results of the object detection method were initially promising, but detailed inspection showed that the performance was not
...
\begin{tabular}{||c c c||} 
 ...
 Simple CNN (Classification) & TODO & TODO \\
 Simple CNN (Regression) & TODO & TODO \\
 Transfer Learning of Pretrained ResNet18 (Classification) & TODO & TODO \\
 Transfer Learning of Pretrained ResNet18 (Regression) & TODO & TODO \\
 Simple CNN (Regression) & TODO & TODO \\
 88 & 788 & 6344 \\ [1ex] 
 \hline
\end{tabular}
