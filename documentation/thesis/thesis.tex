...
\begin{figure}[ht]
	\centering
  	\includegraphics[width=9\textwidth]{graphics/simple_neural_network.png}
...
If a neural network is used with multiple layers, where each layer applies some form of higher level feature extraction compared to the earlier layers it is called deep learning. Such neural network is also called deep neural network.
...
$\alpha > 0$ is a hyperparameter and called the learning rate. 


\subsubsection{Neural Network Tips \& Tricks}
...
The more complex a task is, the more labelled data deep learning needs. This is not only expensive, but sometimes not possible. Gary Larson for example has only drawn so many cartoons in his lifetime. Labelling a big data set is also a big challenge and not always feasible.

This is where transfer learning shines. Instead of training a completely new model for each problem task, the idea is to use TODO
