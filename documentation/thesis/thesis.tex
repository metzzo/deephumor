...
\begin{enumerate}
\item \textbf{LSTM / GRU}: Overfitting showed to be a big problem early on
\item \textbf{Wasserstein Loss instead of Cross Entropy Loss}: Idea was to penalize near misses less also in the classification setting
\item \textbf{Applying a discrete cosine transformation (DCT)}: Since convolutions are not well suited for line drawings the idea was to apply a DCT. 
\end{enumerate}
