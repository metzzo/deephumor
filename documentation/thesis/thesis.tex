...
This chapter aims to evaluate the performance of the implemented architectures. in general the results show that this problem is a very hard one and no architecture was able to deliver results better than the baseline. \\
...
The two main metrics chosen are mean absolute error (MAE) and accuracy. The MAE is being used because it does not penalize cases which are near misses as much. If the model predicts a funniness of 5, while the real funniness is 6 it is not as bad as it would have predicted a 1.


\section{Architectures}
Several architectures have been implemented and are evaluated in the following.

\subsection{Baseline}

Three baseline strategies were picked: 

\begin{itemize}

\item Most Frequent Funniness: Picks the most frequent label in the dataset
\item Average Funniness: Returns the average funniness of the dataset
\item Random Funniness: Returns a random funniness picked uniformly

\end{itemize}

For the mean absolute error metric the average performed best, while for accuracy the most frequent class has the best score.

\subsection{Simple CNN}
The simple CNN architecture overfits very quickly. The accuracy is very similar to the most frequent baseline, which indicates that the model probably just learns to return the most frequent class.

\subsection{Transfer Learning of Pretrained ResNet18}
Noteworthy about this approach is that also enabling the data augmentation in the training phase makes this approach better than the baseline. The exact reason is not obvious, but indicates that the model does not generalize enough for the "not" augmented training samples.

\subsection{Transfer Learned Object Detection}

The result of the object detection approach are neglectable. Since the performance did not contain any noteworthy patterns it was not even implemented far enough to have any performance metrics. The predicted objects for each frame were essentially random.

\subsection{ELMo Pretrained Model}
TODO

\subsection{AutoML Model}
This model performed surprisingly well, even though it does not use any Deep Learning. TODO

\subsection{Two Stage Model}
TODO

\subsection{Two Stage Model with Cartoons}
TODO

\subsection{Two Stage Model with Autoencoder for Cartoons}
TODO

\subsection{Evaluation Conclusion}

In general the conclusion is that no model really learns to generalize the humour. There is no significant improvement compared to the baseline. TODO
...
\begin{tabular}{||c c c||} 
 \hline
 Architecture & Mean Absolute Error & Accuracy \\ [0.5ex] 
 \hline\hline
 Baseline Most Frequent & 2.14 & 25.20\% \\ 
 Baseline Average & 1.53 & 13.94\% \\
 Baseline Random & 2.16 & 12.06\% \\
 Simple CNN & TODO & TODO \\
 Transfer Learning of Pretrained ResNet18 & TODO & TODO \\
 Transfer Learning of Pretrained ResNet18 (enabling augmentation) & TODO & TODO \\
 Simple CNN (Regression) & TODO & TODO \\
 88 & 788 & 6344 \\ [1ex] 
 \hline
\end{tabular}
